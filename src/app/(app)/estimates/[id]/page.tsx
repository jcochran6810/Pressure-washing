...
import { setEstimateStatus, convertEstimateToInvoice, deleteEstimate, saveEstimateToDrive } from "../actions";
...
export default async function EstimateDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isLocked = est.status === "accepted" || est.status === "converted";
  const canMarkResponse = est.status === "draft" || est.status === "sent";
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 mt-2 mb-5">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold">{workflowLabel(est.estimate_number, {
            hasJob: !!workflow.jobId,
            hasInvoice: !!workflow.invoiceId,
        ...
        <div className="flex flex-wrap gap-2">
          {!isLocked && <Link href={`/estimates/${est.id}/edit`} className="btn-secondary">Edit</Link>}
          <a href={`/api/documents/estimates/${est.id}/pdf`} target="_blank" rel="noopener" className="btn-secondary">Print / View</a>
          <form action={saveDrive}><button className="btn-secondary">Save to Drive</button></form>
          {canMarkResponse && <form action={markAccepted}><button className="btn-secondary">Mark accepted</button></form>}
          {canMarkResponse && <form action={markDeclined}><button className="btn-ghost">Mark declined</button></form>}
        </div>
      </div>
      ...
      <NextStepBanner
        ...
        customerHasEmail={!!cust?.email}
        customerHasPhone={!!(cust?.phone || cust?.mobile_phone)}
      />
      ...
      <div className="mt-5 flex flex-wrap gap-2 items-center">
        {est.status !== "converted" && (
          <form action={convert}>
            <button className="btn-ghost text-xs text-gray-600">Convert directly to invoice →</button>
          </form>
        )}
        <form action={del}>
